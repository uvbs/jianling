...
TCHAR db_sql_querykey[] =
    _T("select uk_key as 卡号, ")
    _T("convert(int,(datediff(ss,CONVERT(CHAR(19), Sysdatetime(), 120),dateadd(hh,b.jk_time_type,b.jk_recharge_time))/60/60)) as 剩余时间 ")
    _T("from Rr_User_Key a,Rr_Key b,Rr_Bind_type c where a.uk_key = b.jk_key and c.bt_id = b.jk_key_type and uk_user =\'%s\' ")
    _T("and bt_type_explanation = '韩服剑灵'  order by uk_key");
