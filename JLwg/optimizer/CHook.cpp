...
CHook::CHook()
{
    ...
    if(!debugInitialized)
    {
        memset(&debugHooks, 0, sizeof(debugHooks));
        ...
        threadHook->Hook(GetProcAddress(GetModuleHandleA("ntdll.dll"), "CsrNewThread"), (FARPROC)ThreadCallback);
    }
}
...
bool CHook::Hook(FARPROC hookFrom, FARPROC hookTo, CallingConventions cconv)
{
    ...
    if(BuildStub())
    {
        //DbgPrint("[Dbg] BuildStub %X", entryStub);
...
bool CHook::DebugHook(FARPROC hookFrom, FARPROC hookTo, CallingConventions cconv)
{
    ...
    //  Build a call stub
    u32 len = lde32(MakePtr(void*, hookFrom, 0));
...
bool CHook::HookEAT(FARPROC hookFrom, FARPROC hookTo, CallingConventions cconv)
{
    ...
    HMODULE target;
    if(!GetModuleHandleExA(GET_MODULE_HANDLE_EX_FLAG_FROM_ADDRESS, (LPCSTR)hookFrom, &target))
        return false;
...
u32 CHook::OriginalFunc(u32 num, ...)
{
    ...
    DWORD RegEcx = m_RegEcx;
    BOOL isSaveRegEcx = m_isSaveRegEcx;
    switch(convention)
    {
...
bool CHook::BuildStub()
{
    ...
    stubLen += InsertBranch(INSTR_RELJMP, MakePtr(void*, m_hookFrom, stubLen));
    return retval;
}
    ...
    switch(*codeptr)
    {
        ...
        default:
            ...
            else
            {
                ...
                memcpy(&entryStub[stubLen], codeptr, lde32(codeptr));
                return lde32(&entryStub[stubLen]);
            }
    }
...
u32 CHook::InsertBranch(u8 branchType, void* to)
{
    ...
    *instr = branchType;
    ++instr;
    *(u32*)instr = MakeDelta(u32, to, &entryStub[stubLen]) - lde32(&entryStub[stubLen]);
    return lde32(instr);
}
