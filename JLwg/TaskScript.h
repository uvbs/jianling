...
public:
	...
	void Task_904(int zid,int taskid);//4장. 환귀와 붉은 꽃
	void Task_905(int zid,int taskid);//5장. 경천맹주의 명
	void Task_906(int zid,int taskid);//6장. 바람의 늑대
	void Task_907(int zid,int taskid);//7장. 깨어난 신시
	void Task_908(int zid,int taskid);//8장. 행방불명된 회주
	void Task_931(int zid,int taskid);//9장. 귀농하는 강호인
	void Task_909(int zid,int taskid);//10장. 돼지와 인간 사이
	void Task_910(int zid,int taskid);//11장. 위기의 일심
	void Task_911(int zid,int taskid);//12장. 다시 만난 충각단
	void Task_912(int zid,int taskid);//13장. 그림자 맹주
	void Task_913(int zid,int taskid);//14장. 개구리의 왕
	void Task_914(int zid,int taskid);//15장. 최강의 전투종족
	void Task_915(int zid,int taskid);//16장. 귀도시를 지키는 자들
	void Task_916(int zid,int taskid);//17장. 장수의 망령
	void Task_917(int zid,int taskid);//18장. 탁기에 물든 백성들
	void Task_918(int zid,int taskid);//19장. 왕이 되는 의식
	void Task_919(int zid,int taskid);//20장. 천하사절의 등장
	void Task_921(int zid,int taskid);//21장. 익산운의 아들
	void Task_922(int zid,int taskid);//22장. 원숭이 왕과 비무
	void Task_923(int zid,int taskid);//23장. 경천패의 주인
	void Task_926(int zid,int taskid);//24장. 고립된 병사들
	void Task_924(int zid,int taskid);//25장. 위기의 경천맹
	void Task_925(int zid,int taskid);//26장. 제국의 대용맥
	void Task_927(int zid,int taskid);//27장. 대봉기
	void Task_928(int zid,int taskid);//28장. 마도의 길
	void Task_929(int zid,int taskid);//
