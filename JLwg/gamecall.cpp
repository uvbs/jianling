...
void Gamecall::Fuhuo(DWORD uiAddr)
{
    ...
    __try
    {
        log.info(_T("进入复活call uiAddr:%d"), uiAddr);
        __asm
        {
            mov eax, uiAddr;
            push eax;
...
void Gamecall::HeChengWuQi(_BAGSTU& zhu, _BAGSTU& fu)
{
    ...
    log.info(_T("%s: %08x, %08x"), FUNCNAME, zhu, fu);
    __try
    {
        _asm
        {
...
BYTE Gamecall::GetObjectType(DWORD pObjAddress)
{
    ...
    __except(1)
    {
        //log.info(_T("GetObjectType-error:%d"),pObjAddress);
        OutputDebugString(FUNCNAME);
        return 0;
    }
...
BOOL Gamecall::isHaveXianlu(int index)
{
    ...
    if(uiAddr == 0)
    {
        log.info(_T("isHaveXianlu 获取ui地址失败"));
        return FALSE;
    }
...
void Gamecall::CloseShangDian()
{
    ...
    {
        log.info(_T("%s: 获取商店UI地址失败"), FUNCNAME);
    }
...
void Gamecall::_PickupTask(ObjectNode* pObj)
{
    ...
    log.info(_T("%s: id: %d, id2: %d"), FUNCNAME, id1, id2);
    __try
    {
        __asm
        {
...
{
    //log.info(_T("格子数:%d"),bag.m_Info);
    int value = bag.m_Info;
    value <<= 16;
    value += package;

    //log.info(_T("fenjie.value:%d"),value);
    try
    {
        __asm
        {
...
void Gamecall::ChiYao(const wchar_t* name)
{
    ...
    {
        log.info(_T("%s: 没有在背包中找到这个物品: %s"), FUNCNAME, name);
    }
...
BOOL Gamecall::ConfirmAgreement()
{
    ...
    if(bUiok == FALSE)
    {
        log.info(_T("没有在设置的时间内找到登录协议的UI"));
        return FALSE;
    }
...
BOOL Gamecall::CloseAttendance()
{
    ...
    if(buiOpen == FALSE)
    {
        log.info(_T("没有在设置的时间内等待到摇奖UI显示"));
        return FALSE;
    }
...
void Gamecall::GetRangeMonsterToVector(DWORD range, std::vector<ObjectNode*>& MonsterVec)
{
    try
    {
        ...
        {
            ObjectNode* pNode = RangeObject[i];
            //log.info(_T("执行GetObjectName"));


            fPosition fpos;
            //过滤掉距离远的和没距离的
            //log.info(_T("执行GetObjectPos"));
            if(GetObjectPos(RangeObject[i], &fpos) == FALSE) continue;
...
BOOL Gamecall::GetStrikeByName(const wchar_t* name, STRIKEINFO* pStrikeInfo)
{
    ...
    log.info(_T("%s: 没有找到对应技能"), FUNCNAME);
    return FALSE;
}
...
{
    log.info(_T("add:%x,name:%s,fanhui:%x"), jiegou.adress, jiegou.name, jiegou.ID);
...
void Gamecall::_GetUiAddrByName(Tree* Addr, wchar_t* name, DWORD& reAddr)
{
    ...
    if(!IsBadReadPtr((void*)GetUiNewName(Addr->Adress), sizeof(DWORD)))
    {
        ...
        {
            //TRACE(_T("找到需要的控件了"));
            //log.info(_T("找到UI名:%s,找到控件地址:%d"),uiname,Addr->Adress);
            reAddr = Addr->Adress;
            return ;
        }
    }
