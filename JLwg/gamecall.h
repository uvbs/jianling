...
public:
    ...
    DWORD DuiWu_StartAdress();

    //队伍开始地址
    BOOL GetPartyInfo(TeamVector& TeamInfo);

    //获取队伍信息
    DWORD GetPartyByAddress(DWORD PartyAddress, int i);

    //获取当前角色地址
    wchar_t* GetPartyName(DWORD PartyAddress);

    //获取队伍角色名
    DWORD GetPartyChannel(DWORD PartyAddress);

    //获取角色当前频道
    DWORD GetPartyCurrlife(DWORD PartyAddress);

    //获取角色当前血值
    DWORD GetPartyMaxlife(DWORD PartyAddress);

    //获取角色最大血值
    DWORD GetPartyId(DWORD PartyAddress);

    //获取角色id
    DWORD GetPartyId2(DWORD PartyAddress);

    //获取角色id2
    fPosition GetPartyPos(DWORD PartyAddress);

    //获取角色x坐标
    DWORD GetPartyAngle(DWORD PartyAddress);
